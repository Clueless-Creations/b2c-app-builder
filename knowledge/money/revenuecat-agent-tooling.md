...
| `customers simulate-purchase` | `{ app_id, app_user_id, product, fetch_token, customer_info, active_entitlements }` | Product id and store identifier, SDK entitlement identifiers, `TEST_` fetch token | Native IAP, in-app UI, App Store/Play. `active_entitlements` are lookup keys, not management `entl_*` ids. Test Store completeness unions those keys with `customers.show`; empty expected entitlements stay incomplete. |
...
| Test Store purchase | `rc --project-id <id> customers simulate-purchase --app-id <test-store-app> --product <id> --app-user-id <id> --yes --json --no-input --no-color` | remote mutation | refused unless `apps show` reads a Test Store and host authority is granted; caller labels are not that proof; complete only when expected product and entitlement lookup keys match simulate-purchase and/or `customers.show`; `customers.show` is not the only readback; not native IAP proof |
