...
Use the local `b2c-local` MCP server from this repository for workspace planning and execution. Hosted knowledge registers as `b2c-hosted` and cannot see or run a local business. When both are configured, select by that capability; a leftover `b2c-app-builder` name is not a third surface. Duplicate names are a collision. A missing worker CLI degrades local execution health; it does not turn this connection into hosted knowledge. Do not route consumer-app work through Planes.
