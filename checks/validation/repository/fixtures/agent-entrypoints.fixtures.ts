...
export function register(harness: Harness): void {
  ...
  harness.runScriptArgs("agent entrypoints reject an adapter that restates a canonical section", SCRIPT, ["--repo-root", restated], 1, "agent_entrypoints.adapter_duplicates_heading");
  harness.runScriptArgs("agent entrypoints name the restated prose line as well as the heading", SCRIPT, ["--repo-root", restated], 1, "agent_entrypoints.adapter_duplicates_prose");
  ...
  harness.runScriptArgs("agent entrypoints reject a workspace template that names an ARCH rule or roadmap unit", SCRIPT, ["--repo-root", leaked], 1, "agent_entrypoints.internal_reference");
  ...
  harness.runScriptArgs("agent entrypoints reject a workspace rule that names a builder repository path", SCRIPT, ["--repo-root", repositoryPath], 1, "agent_entrypoints.internal_reference");
  ...
  harness.runScriptArgs("agent entrypoints reject an adapter that drops its AGENTS.md pointer", SCRIPT, ["--repo-root", unpointed], 1, "agent_entrypoints.adapter_pointer_missing");
  ...
  harness.runScriptArgs("agent entrypoints reject a root guide that drops one scope's router", SCRIPT, ["--repo-root", unrouted], 1, "agent_entrypoints.scope_router_missing");
  ...
  harness.runScriptArgs("agent entrypoints reject a skill index that omits one skill", SCRIPT, ["--repo-root", unlisted], 1, "agent_entrypoints.skill_index_incomplete");
  ...
  const read = (relative: string): string | undefined => {
    try { return readFileSync(path.join(control, relative), "utf8"); }
    catch (error) {
  ...
  const record = (label: string, verify: () => void): void => {
    let output = "";
    try { verify(); } catch (error) { output = String(error); }
  ...
  for (const term of ["b2c-local", "b2c-hosted", "Duplicate names are a collision", "Degraded execution still selects b2c-local", "CLI-only public MCP names", "write-gated MCP names", "mcp_readonly", "Hosted leftover names stay wrong-surface"]) {
  ...
  record("injected files count in full, including text after an early-exit instruction", () => {
    ...
    if (result.utf8Bytes !== Buffer.byteLength(text, "utf8") || result.utf8Bytes === text.length) throw new Error("Byte accounting lost injected or multibyte text");
  });
  record("missing packet files stay unmeasured rather than zero bytes", () => {
    let refused = false;
    try { measureGuidancePackets([{ id: "missing", measuredFiles: ["AGENTS.md"], fileCount: 1, utf8Bytes: 0 }], () => undefined); }
    catch { refused = true; }
  ...
  harness.runScriptArgs("A0 packets replay through the existing entrypoint owner", SCRIPT, ["--repo-root", skillRoot, "--guidance-baseline", "docs/research/agent-guidance-a0.json", "--guidance-report", report], 0);
  record("A0 replay preserves all ten identities and does not invent model evidence", () => {
    const actual = JSON.parse(readFileSync(report, "utf8")) as { cases: Array<{ id: string }>; modelId: unknown; modelTokens: unknown; observedAgentTrace: unknown; serviceResult: unknown };
    if (actual.cases.map((item) => item.id).join() !== Array.from({ length: 10 }, (_, index) => `A0-${String(index + 1).padStart(2, "0")}`).join()) throw new Error("A0 cases changed");
    if ([actual.modelId, actual.modelTokens, actual.observedAgentTrace, actual.serviceResult].some((value) => value !== null)) throw new Error("Unobserved model/service evidence was populated");
  });
}
