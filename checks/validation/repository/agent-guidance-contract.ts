...
export const STANDING_RULES: ReadonlyArray<{ id: string; terms: readonly string[] }> = [
  { id: "privacy", terms: ["issues are public", "secret values", "live provider/operator identifiers", "private-repository references", "personal addresses", "home paths", "machine names", "customer/operator status", "production deployment configuration", "without quoting the value publicly"] },
  { id: "architecture", terms: ["target architecture", "implementation evidence", "truth ownership", "dependency direction", "migration guarantees", "architecture decision", "Compatible internal choices do not"] },
  { id: "single_owner", terms: ["existing owners", "Do not add competing", "not another knowledge graph", "one logical routing model"] },
  { id: "truth", terms: ["intent losslessly", "`product.yaml` owns", "`PRODUCT.md` is rendered", "`DESIGN.md` owns", "Reducer-owned state owns", "only through the reducer", "Git owns", "registry owns workspace identity/address"] },
  ...
  { id: "authored_generated", terms: ["canonical authored guides, thin host adapters, and generated projections", "nearest applicable AGENTS.md", "Nested guides may narrow scope", "Edit authored", "before rendering", "stable public/catalog/reference IDs", "public major version"] },
  { id: "workspace_boundary", terms: ["Workspace-facing guidance", "never maintainer ARCH rules", "contributor machinery"] },
  { id: "completion", terms: ["An implementation request authorizes", "in-scope, reversible repository work", "repair failures caused by the change", "regenerate affected output", "inspect the diff", "complete required checks without repeated intermediate approvals", "does not authorize unrelated fixes", "Review-only and planning-only requests remain review and planning, not implementation"] },
  { id: "authority", terms: ["Reuse valid existing scoped authority", "neither a runtime grant nor an approval receipt", "pause if it is absent or insufficient", "not safe merely because it runs locally in a terminal"] },
  { id: "protected_effects", terms: ["external account/access", "credential changes", "spend", "pricing or legal decisions", "destructive actions", "deployment", "publication", "store submission", "production release"] },
  { id: "provider_upstream", terms: ["explicit provider bindings", "actual tool availability", "Provider adapters implement canonical operations", "guidance is subordinate", "cannot add requirements", "change provider selection/state", "override evidence", "redefine completion"] },
  { id: "verification", terms: ["CONTRIBUTING.md", "all applicable required gates before merge", "presubmit is not a full-audit pass", "Do not skip suites, weaken CI", "current provider/device/store/runtime evidence"] },
  { id: "integration", terms: ["non-overlapping ownership", "delegation is not mandatory", "One coordinating agent owns shared-file integration", "final verification", "genuinely independent of implementation", "do not invent a reviewer"] },
];
...
export function checkStandingGuidance(read: ReadGuidance): GuidanceFinding[] {
  ...
  if (!portableRule || !skill.includes(portableRule)) {
    add("portable_completion_drift", SKILL, "The portable completion rule must match the authored standing rule without requiring repository AGENTS.md at runtime.");
  }
  ...
  for (const term of ["Local b2c-local", "hosted b2c-hosted", "read-only knowledge", "cannot execute a local business", "Missing execution tooling does not block advisory work"]) {
  ...
  if (setup === undefined || !skill.includes(`](${SETUP})`)) {
    ...
    for (const term of ["b2c-local", "b2c-hosted", "leftover `b2c-app-builder`", "Duplicate names are a collision", "missing worker CLI", "Degraded execution still selects b2c-local", "CLI-only public MCP names", "write-gated MCP names", "mcp_readonly", "Hosted leftover names stay wrong-surface", "b2c inspect", "b2c doctor", "Keep the MCP read-only by default", "Do not edit an agent configuration or install software unless the user requested setup"]) {
...
export function measureGuidancePackets(packets: readonly GuidancePacket[], read: ReadGuidance): Array<{
  id: string;
  fileCount: number;
  utf8Bytes: number;
