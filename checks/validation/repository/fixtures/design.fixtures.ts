...
function writeValidDesignExploration(root: string): void {
  ...
  process:
    ...
    tangibleDraft: A low-hierarchy canvas draft was rendered with the three surface treatments and a first onboarding path.
    critique: A concrete review found that the first draft hid omissions on narrow screens and did not carry the empty-state cue into onboarding.
    revision: The selected canvas direction added empty silhouettes, clarified the primary action, and carried the same object model through onboarding.
