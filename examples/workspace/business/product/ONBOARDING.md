...
Record `greenfield`, `replacement`, `audit_only`, or founder-scoped `incremental`. Name the product surfaces, freshness date, owner, durable user value, and whether replacement requires hard cutover and only the target runtime.
...
| ----------- | ------------------------------------------------- | --------------- | ---------------- | ---------------------------------------------------------- | --------------- |
| `GUIDE-001` | Record the B2C App Builder or internal B2C source | Record the rule | Record the basis | Pass, partial, fail, outdated, conflict, or not applicable | Record          |
...
| -------------------- | -------- | ------------------ | ------------------------------------------- | ------ |
| welcome              | Yes      | Record             | Fake scarcity veto                          | Record |
| quiz or goals        | Yes      | Record             | Useless questions veto                      | Record |
| micro-commitment     | Yes      | skip path required | Pledge or hold-to-confirm without skip path | Record |
| personalized insight | Yes      | Record             | Progress animation over no computation      | Record |
| hard paywall         | Yes      | Restore visible    | Fake download counts veto                   | Record |

## Funnel Quality Metrics

Trial starts alone cannot mark onboarding done. Record quality companions and route tests through RevenueCat Experiments.

| Metric              | Definition | Current value | Quality bar | Experiment |
| ------------------- | ---------- | ------------- | ----------- | ---------- |
| onboarding complete | Record     | Record        | Record      | Record     |
| install-to-trial    | Record     | Record        | Record      | Record     |
| trial-to-paid       | Record     | Record        | Record      | Record     |
| per-step drop-off   | Record     | Record        | Record      | Record     |
| refund rate         | Record     | Record        | Record      | Record     |
...
| -------------- | ------------------ | --------------------------- | --------------------------- |
| `lose_weight`  | Record             | Start losing weight today   | Record the generic fallback |
| `build_muscle` | Record             | Start building muscle today | Record the generic fallback |
| `skipped`      | Skip or blank      | Record the generic fallback | Yes                         |

Runtime bind: Offering metadata holds the template map. `customVariables` receive `paywall_headline_key`. Sync subscriber attributes when Experiments targeting needs the key.
...
| Task ID        | Outcome and paths                            | Dependencies and contracts | Analytics and provider work | Tests  | Removal tasks | Acceptance |
| -------------- | -------------------------------------------- | -------------------------- | --------------------------- | ------ | ------------- | ---------- |
...
| ------------ | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------- | ------------------------------- |
| `REMOVE-001` | Record the item                                                                 | Preserve durable data, transform once, recompute, archive legal history, or delete | Record      | Record the search or validation |
...
- [ ] `checks/validation/business/experience/check-onboarding-graph.ts` passes
