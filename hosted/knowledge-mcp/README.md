...
receives an OAuth token, not an entitled API key. Keep the local `b2c` entry if you use local
execution. Configure other clients with the same remote URL and OAuth authorization.
