...
| #75 | Partial Stage A walk: real `createKnowledgeService()`, `brief: true`, `nextCall` reassembly, and reserved held-out paraphrases (`store-001`, `store-002`). | Full Stage A corpus split / per-case report still open. Paid Stage B live-worker evaluation. No vector store. |
