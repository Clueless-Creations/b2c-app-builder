...
| RevenueCat CLI        | Schema discovery, exact project selection, setup-effect review, and a typed host-executable adapter for selected operations | No host CLI was installed or authenticated. Support is qualified against release v0.1.1, not the later source head. `rc` is not Rork's `asc`. |
