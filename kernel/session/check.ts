 ...
 * gives a maintainer.
 *
 * Packed omit-dev prefers `dist/` for `check:credits` (`tooling/render-credits.ts`) and
 * `check:hosted-bundle` (`tooling/render-hosted-bundle.ts`). Every other `check:*` stays on
 * the remaining-tsx inventory — including `render-public-api` and the uncompiled `checks/` graph.
 *
