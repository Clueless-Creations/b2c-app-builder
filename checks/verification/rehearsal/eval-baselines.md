...
| #75 | Pin the older combined DESIGN.md `nextCall` walk to the artifact-contract heading. Stale-hash uses a controlled changed bundle (`revision_mismatch`), not a zero hash. | Stage A still open: no retrieval change adopted, held-out unused for tuning. Paid Stage B. No vector store. |
...
The older combined walk now uses the same pinned artifact-contract `DESIGN.md`
heading as the per-case artifact/stale/repeated classes. Stale-hash clones the
hosted bundle, edits that heading, and requires `revision_mismatch` on the prior
hash. A zero hash is not a changed bundle.
