...
export function register(h: Harness): void {
  h.check("packed-check: remaining-tsx inventory matches authored check scripts", () => {
    ...
    assert(
      compiled.join(",") ===
        "check:agent-entrypoints,check:architecture,check:autopilot,check:capability-delta,check:catalog,check:credits,check:gates-layout,check:graph-foundations,check:hosted-bundle,check:hub-spoke,check:learning-grounding,check:operating-graph,check:pack-composition,check:package-parity,check:public-api,check:validator-docs",
