...
test("writes to interest_signals, keyed on lower(email)", async () => {
  ...
  await handleInterestSubmission(valid, { accountId: "acct_1", country: "US" }, db, analytics, ctx);
...
test("an authenticated submission is stored and then captured", async () => {
  ...
  try {
    assert.deepEqual(await handleInterestSubmission(valid, { accountId: "acct_1", country: "US" }, db, analytics, ctx), { status: 200 });
...
test("a signed-out submission is first-class — stored with a null account and captured anonymously", async () => {
  ...
  try {
    ...
    assert.deepEqual(await handleInterestSubmission(valid, { distinctId: "anon_abc", country: "US" }, db, analytics, ctx), { status: 200 });
...
test("with no identity at all the row is still stored, but no event is fabricated", async () => {
  ...
  const stub = stubFetch();
  try {
    assert.deepEqual(await handleInterestSubmission(valid, { country: "US" }, db, analytics, ctx), { status: 200 });
...
test("a D1 failure rejects the submission and emits nothing", async () => {
  ...
  const stub = stubFetch();
  try {
    assert.deepEqual(await handleInterestSubmission(valid, { accountId: "acct_1", country: "US" }, db, analytics, ctx), {
      status: 500,
...
test("timestamps match the migration's strftime CHECK format", async () => {
  const db = stubDb();
  const { ctx } = collectingCtx();
  await handleInterestSubmission(valid, { accountId: "acct_1", country: "US" }, db, analytics, ctx, new Date("2026-09-01T18:00:00Z"));
...
test("bounds match the stricter D1 writer, since 0006 has no CHECK constraints to catch drift", async () => {
  ...
  assert.deepEqual(await handleInterestSubmission({ ...valid, initial_utm_source: "x".repeat(201) }, { country: "US" }, db, analytics, ctx), {
    status: 400,
    error: "invalid_submission",
  });
  assert.deepEqual(await handleInterestSubmission({ ...valid, initial_utm_source: "x".repeat(200) }, { country: "US" }, db, analytics, ctx), { status: 200 });
  // referral_code carries a charset, not just a length.
  assert.deepEqual(await handleInterestSubmission({ ...valid, referral_code: "has spaces" }, { country: "US" }, db, analytics, ctx), {
    status: 400,
    error: "invalid_submission",
  });
  assert.deepEqual(await handleInterestSubmission({ ...valid, referral_code: "ok_code-1" }, { country: "US" }, db, analytics, ctx), { status: 200 });
});

test("the account is server-resolved — a body-supplied account_id is rejected", async () => {
  const db = stubDb();
  const { ctx } = collectingCtx();
  const result = await handleInterestSubmission({ ...valid, account_id: "acct_victim" }, { accountId: "acct_1", country: "US" }, db, analytics, ctx);
...
test("unknown source keys, intents, emails and over-long text are refused", async () => {
  ...
  for (const body of [
    ...
    assert.deepEqual(await handleInterestSubmission(body, { accountId: "acct_1", country: "US" }, db, analytics, ctx), {
      status: 400,
      error: "invalid_submission",
    });
  }
...
test("an EEA/UK submission is stored but never captured — the console's second path", async () => {
  ...
  for (const country of ["DE", "GB", "IE", undefined, "XX", "EU"]) {
    ...
    const stub = stubFetch();
    try {
      assert.deepEqual(await handleInterestSubmission(valid, { accountId: "acct_1", country }, db, analytics, ctx), { status: 200 });
